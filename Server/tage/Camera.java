...
public class Camera
{
	...
	public void pitch(float movementSpeed){
		Vector3f rightVector, upVector, fwdVector;
		rightVector = getU();
		upVector = getV();
		fwdVector = getN();
		fwdVector.rotateAxis(-movementSpeed, rightVector.x(), rightVector.y(), rightVector.z());
		upVector.rotateAxis(-movementSpeed, rightVector.x(), rightVector.y(), rightVector.z());
		setN(fwdVector);
		setV(upVector);
	}

	public void yaw(float movementSpeed){

		Vector3f rightVector, upVector, fwdVector;
		rightVector = getU();
		upVector = getV();
		fwdVector = getN();
		rightVector.rotateAxis(movementSpeed, upVector.x(), upVector.y(), upVector.z());
		fwdVector.rotateAxis(movementSpeed, upVector.x(), upVector.y(), upVector.z());
		setU(rightVector);
		setN(fwdVector);
	}

	public void moveToFro(float movementSpeed){
		Vector3f loc, fwd, newLocation;
		fwd = getN();
		loc = getLocation();
		newLocation = loc.add(fwd.mul(movementSpeed));
		setLocation(newLocation);
	}
	public void positionCameraOnTopOf(GameObject go){
		...
		this.setV(up);
		this.setN(fwd);
		this.setLocation(loc.add(up.mul(1.3f)).add(fwd.mul(-2.5f)));
	}
	public void truck(float movementSpeed){
		Vector3f rightVector, loc, newLocation;
		rightVector = getU();
		loc = getLocation();
		newLocation = loc.add(rightVector.mul(-movementSpeed));
		setLocation(newLocation);
	}
	public void pedestal(float movementSpeed){
		//untested
		Vector3f upVector, loc, newLocation;
		upVector = getV();
		loc = getLocation();
		newLocation = loc.add(upVector.mul(-movementSpeed));
		setLocation(newLocation);
	}
}